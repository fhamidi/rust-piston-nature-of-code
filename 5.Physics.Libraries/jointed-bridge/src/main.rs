...
impl Bridge {
    fn new(world: &mut World, x: f32, y: f32, width: f32) -> Self {
        ...
        for i in 1..count {
            world.create_joint(&b2::DistanceJointDef {
                                   ...
                                   damping_ratio: 0.42,
                                   length: STEP,
                                   ..b2::DistanceJointDef::new(tokens[i - 1]
                                                                   .body_handle(),
    ...
    fn render_joints(&self,
        ...
        for i in 1..self.tokens.len() {
            let start = vertices.len() as u32;
            let bodies = [world.body(self.tokens[i - 1].body_handle()),
...
impl PistonApp for App {
    ...
    fn draw(&mut self, window: &mut PistonAppWindow, state: &PistonAppState) {
        ...
        self.bridge.as_ref().unwrap().extend_vertex_buffer(world,
                                                           texture_atlas,
                                                           &mut self.vertices,
                                                           &mut self.indices);
        for brick in &self.bricks {
            brick.extend_vertex_buffer(world,
                                       texture_atlas,
                                       &mut self.vertices,
                                       &mut self.indices);
        }
